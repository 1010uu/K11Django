...
path() : route, view 2개의 필수인자와 kwargs, name 2개의 선택인수를 받는다.
    route : URL 패턴을 표현하는 문자열로 URL스트링이라고 한다.
    view : URL 스트링이 매치되면 호출되는 View함수. HttpRequest 객체와
        URL스트링에서 추출된 항목이 View 함수의 인자로 전달된다.
    kwargs : URL 스트링에서 추출된 항목 외에 추가적인 인자를 View 함수로 전달할 때
