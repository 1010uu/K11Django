...
'''
해당 URLconf는 프로젝트 레벨이므로 livepolls앱의 views파일을
임포트해야만 접근할 수 있다.
'''
...
'''
장고에서 URL패턴 작성법 
    path(요청URL패턴, 처리할 view의 메서드 함수명, URL의 별칭)
'''
#외부에 있는 urls.py를 인클루드하기 위한 임포트
from django.urls import path
...
urlpatterns = [
    ...
    path('books/', include('books.urls')),          #앱3:도서관리앱
]
'''
    방법1의 경우 Url패턴이 변경되면 모든 항목을 수정해야 하므로 불편한다.
    하지만 방법2와 같이 파일을 2개로 나누게 되면 하나의 패턴을 위한
    네임스페이스가 생셩되는 개념이므로 한번만 수정하면 돼서 편리하다.
