...
class QuestionForm(forms.Form):
    ...
    '''
    추가적인 속성을 부여할 때에는 위젯에 attrs={'size':30}을 기술한다.
    '''
    form3 = forms.CharField(
        widget=forms.PasswordInput(attrs={'size':30})
    )
    
    #<textarea name="form4" cols="40" rows="10" required id="id_form4"></textarea>0
    form4 = forms.CharField(widget=forms.Textarea) 
    
    '''
    select태그를 포현한다. (1개 선택)
        choices : select의 option태그를 구성하는 데이터로 사용한다.
            순서대로 value와 text로 사용된다.
        initial : 해당 태그의 default값으로 사용한다. 주로 수정에서 활용하면 된다.
    ...
    또한 default값을 표현하기 위해 2개 이상의 항목을 선택할 수 있어야 하므로
    리스트를 사용해야 한다. (멀티 선택)
    ...
    <div id="id_form7">
        <div>
            <label for="id_form7_0"><input type="radio" name="form7" value="red" required id="id_form7_0">
                빨강</label>
        </div>
        <div>
            <label for="id_form7_1"><input type="radio" name="form7" value="green" required id="id_form7_1">
                녹색</label>
        </div>
        ...
    </div>
    ...
    <div id="id_form8">
        <div>
            <label for="id_form8_0"><input type="checkbox" name="form8" value="red" id="id_form8_0">
                빨강</label>
        </div>
        <div>
            <label for="id_form8_1"><input type="checkbox" name="form8" value="green" id="id_form8_1" checked>
                녹색</label>
        </div>
    </div>
    
    추가적인 속성을 부여할 때에는 attrs={'class':'red'}와 같이 기술한다.
    '''
    ...
    #파일 입력 <input type="file" name="form9" required id="id_form9">
