...
def main(request): #views에 함수 정의 시 request내장객체는 필수요소임
    return render(request, 'livepolls/main.html')

#투표앱의 첫번째 화면으로 질문이 리스팅 된다.
def index(request):
    #Question테이블에서 pub_date를 내림차순으로 정렬하여 최근 게시물 5개를 가져옴
...
get_object_or_404(모델클래스, 검색조건)
    : 모델 클래스(테이블)에서 검색조건에 맞는 객체를 조회하여 반환한다.
    만약 조건에 맞는 객체가 없다면 http404를 발생시킨다.
'''
def detail(request, question_id):
    #URL패턴을 통해 전달된 일련번호를 통해 Question테이블을 검색후 반환한다.
    question = get_object_or_404(Question, pk=question_id)
    #조회된 데이터를 딕셔너리로 템플릿으로 반환한다.
    return render(request, 'livepolls/detail.html', {'question':question})

#투표 처리 : 선택한 항목의 vote컬럼을 +1한다.
def vote(request, question_id):
    ...
    try: 
        ...
        #예외발생 시 detail 페이지를 다시 보여준다. 이 때 에러메세지 전달.
